import{h as w,i as n}from"./assets/scroll-btn-CASHhAMA.js";import{a as f}from"./assets/vendor-CQpva9gi.js";function c(t){return t&&typeof t=="string"?t.charAt(0).toUpperCase()+t.slice(1):t}async function l(){const t=JSON.parse(localStorage.getItem("favorites"))||[],e=document.getElementById("wrapper-secnd");if(!t||t.length===0){e.innerHTML='<li class="text-exer"><p>It appears that you havent added any exercises to your favorites yet. To get started, you can add exercises that you like to your favorites for easier access in the future.</p></li>';return}const r=async a=>{try{return(await f.get(`https://your-energy.b.goit.study/api/exercises/${a}`)).data}catch(s){return console.error("Error fetching exercise data",s),null}},o=(await Promise.all(t.map(a=>r(a)))).filter(a=>a!==null);if(o.length>0){const a=o.map(({_id:s,name:u,bodyPart:p,target:g,burnedCalories:m,time:v})=>`
